...
type KanbanColor = 'gray' | 'blue' | 'yellow' | 'orange' | 'green';
...
const TaskCard: React.FC<{
  ...
  const priorityColors: Record<LearningTask['priority'], string> = {
    low: 'border-l-green-500',
    medium: 'border-l-yellow-500',
    high: 'border-l-red-500',
  };

  const difficultyIcons: Record<LearningTask['difficulty'], string> = {
    easy: '⚡',
    medium: '🔥',
    hard: '💎',
  };
      ...
      {task.labels.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-1">
          ...
          {task.labels.length > 3 && (
            <span className="text-xs text-white/60">+{task.labels.length - 3} more</span>
          )}
        </div>
      )}
    </motion.div>
