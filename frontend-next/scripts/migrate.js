...
async function main() {
  try {
        ...
        const modernNode = {
          ...
          content: {
            resources: (legacyNode.resources || []).map((res, idx) => ({
              ...
              difficulty: res.difficulty.toLowerCase().includes('beginner')
                ? 'beginner'
                : res.difficulty.toLowerCase().includes('advanced')
                  ? 'advanced'
                  : 'intermediate',
          ...
          progress: {
            status: !legacyNode.isUnlocked
              ? 'locked'
              : legacyNode.progress === 0
                ? 'available'
                : legacyNode.progress === 100
                  ? 'completed'
                  : 'in_progress',
    ...
    writeFileSync(join(OUTPUT_PATH, 'roadmap.json'), JSON.stringify(modernRoadmap, null, 2));

    console.log('✅ Migration completed successfully!');
    console.log(
      `📊 Created ${modernRoadmap.nodes.length} nodes across ${legacyData.phases.length} phases`
    );
  } catch (error) {
