...
export default function RecallIntegration() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('brainwav-recall-items');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('brainwav-recall-items', JSON.stringify(recallItems));
  }, [recallItems]);
  ...
  const exportToJSON = () => {
    ...
    link.href = url;
    link.download = `brainwav-recall-backup-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
