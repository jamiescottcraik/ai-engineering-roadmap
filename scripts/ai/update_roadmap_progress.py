...
class RoadmapProgressTracker:
    ...
    def get_github_activity(self) -> dict[str, Any]:
    ...
    def update_roadmap_json(self) -> dict[str, Any]:
        ...
        if data_file.exists():
            with open(data_file) as f:
                roadmap_data: dict[str, Any] = json.load(f)
        else:
        ...
        for phase in roadmap_data["phases"]:
            if phase["nodes"]:
                phase["progress"] = sum(node.get("progress", 0) for node in phase["nodes"]) / len(
                    phase["nodes"]
                )
    ...
    def create_initial_roadmap(self) -> dict[str, Any]:
        """Create initial roadmap data structure"""
        return {
            "metadata": {
                "title": "AI Leadership Engineering Roadmap",
                "author": "Jamie Scott Craik",  # cSpell:ignore Craik
...
if __name__ == "__main__":
    ...
    print(
        f"📊 Current overall progress: {sum(p['progress'] for p in roadmap_data['phases']) / len(roadmap_data['phases']):.1f}%"
    )
